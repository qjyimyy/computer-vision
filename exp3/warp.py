...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    #solve out the spherical coordinates
    cos_theta = np.cos(xf)
    sin_theta = np.sin(xf)
    cos_phi = np.cos(yf)
    sin_phi = np.sin(yf)
    xh = sin_theta * cos_phi
    yh = sin_phi
    zh = cos_theta * cos_phi
    #project the points to the z=1 plane (normalize it to plane coordinates)
    xp = xh/zh
    yp = yh/zh
    #correct teh redial distortion
    r2 = xp**2 + yp**2
    xt = xp * (1 + k1*r2 + k2*r2**2)
    yt = yp * (1 + k1*r2 + k2*r2**2)
