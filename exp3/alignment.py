...
def computeHomography(f1, f2, matches, A_out=None):
    ...
    for i in range(len(matches)):
        ...
        #TODO-BLOCK-BEGIN
        A[2*i] = np.array([a_x,a_y,1,0,0,0,-a_x*b_x,-a_y*b_x,-b_x])
        A[2*i + 1] = np.array([0,0,0,a_x,a_y,1,-a_x*b_y,-a_y*b_y,-b_y])
...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    if m == eTranslate:
        while i<nRANSAC:
            index = random.sample(range(0, len(matches)), 4)
            x1,y1 = f1[matches[index[0]].queryIdx].pt
            x2,y2 = f2[matches[index[0]].trainIdx].pt
            tp = np.array([[1, 0, x2-x1], [0, 1, y2-y1], [0, 0, 1]])
            inn = getInliers(f1, f2, matches, tp, RANSACthresh)
            if max_match<inn.__len__():
                best_match.append(inn)
                max_match = inn.__len__()
            i += 1
        ...
        while i<nRANSAC:
            ...
            tp = computeHomography(f1, f2, new_match)
            inn = getInliers(f1, f2, matches, tp, RANSACthresh)
            best_match = inn if max_match < inn.__len__() else best_match
            max_match = max(max_match, inn.__len__())
            i += 1
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        new = []
        for i in range(len(inlier_indices)):
            new.append(matches[inlier_indices[i]])
        M = computeHomography(f1, f2, new)
        #raise Exception("TODO in alignment.py not implemented")
        #TODO-BLOCK-END
        #END TODO
    ...
    return M
"""
def getExposureDiff(img1,img2,f1,f2,matches,m,nRANSAC,RANSACthresh):
    best_match = []
    max_match = -1
    i = 0
    if m == eTranslate:
        ...
        while i<nRANSAC:
            ...
            max_match = max(max_match, inn.__len__())
            i += 1
    return leastDiff(img1,img2,f1, f2, matches, m, best_match)

def leastDiff(img1,img2,f1,f2,matches,m,inlier_indices):
    ED = 0
    if m == eTranslate:
        for i in range(len(inlier_indices)):
            x1, y1 = f1[matches[i].queryIdx].pt
            ...
            x2 = int(x2)
            y2 = int(y2)
            ED += img1[y1,x1] - img2[y2,x2]
        ED /= len(inlier_indices)
    elif m == eHomography:
        new = []
        for i in range(len(inlier_indices)):
            new.append(matches[inlier_indices[i]])
