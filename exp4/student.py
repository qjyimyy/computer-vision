...
def compute_photometric_stereo_impl(lights, images):
    ...
    albedo = np.zeros((height,width,channels)).astype('float32')
    normals = np.zeros((height,width,3)).astype('float32')
    for i in range(height):
        for j in range(width):
            for k in range(channels):
                Illum = np.array([[img[i,j,k] for img in images]])
                G = np.linalg.inv(lights.T @ lights) @ (lights.T @ Illum.T)
                albedo[i,j,k] = np.linalg.norm(G)
                normals[i,j,:] += G.T[0]
            if np.linalg.norm(albedo[i,j,:]) < 1e-7:
                albedo[i,j,:] = 1
                normals[i,j,:] = 0
            else:
                normals[i,j,:] /= np.sum(albedo[i,j,:])
...
def preprocess_ncc_impl(image, ncc_size):
    ...
    for i in range(height):
        for j in range(width):
            if i>=border and j>=border and i<height-border and j<width-border:
                mean = np.mean(np.mean(image[i-border:i+border+1,j-border:j+border+1,:], axis=1), axis=0)
                patch = image[i-border:i+border+1,j-border:j+border+1,:]-mean
                if np.linalg.norm(patch)<1e-5:
                    normalized[i,j,:] = 0
                else:
                    divisor = np.sqrt(np.sum(patch**2))
                    if divisor == 0:
                        divisor = 1
                    patch = patch/divisor
                    patch = patch.transpose(1,0,2).reshape((1,-1),order = 'F')
    ...
    if normals is not None:
        for i in range(height-1):
            for j in range(width-1):
                if alpha[i,j] != 0 and alpha[i+1,j] != 0 and alpha[i,j+1] != 0:
